...
fun WishlistPendingScreen(
    ...
    val pendingBooks = wishlist.mapNotNull { bookId ->
        val book = BookRepository.getBookById(bookId)
        val currentPage = progressMap[bookId] ?: 0
        if (book != null && currentPage < 1) Pair(book, currentPage) else null
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            LazyColumn(
                ...
                items(pendingBooks.size) { index ->
                    val (book, currentPage) = pendingBooks[index]
                    WishlistBookCard(
                        book = book,
